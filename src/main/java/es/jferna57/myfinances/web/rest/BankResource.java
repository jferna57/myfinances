...
/**
 * REST controller for managing {@link es.jferna57.myfinances.domain.Bank}.
 */
@RestController
@RequestMapping("/api")
public class BankResource {
     ...
     */
    @PutMapping("/banks/{id}")
    public ResponseEntity<Bank> updateBank(@PathVariable(value = "id", required = false) final String id, @Valid @RequestBody Bank bank)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, bank.getId()))
            .body(result);
    }
    ...
    public ResponseEntity<Bank> partialUpdateBank(
        ...
        return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, bank.getId()));
    }
     ...
     */
    @GetMapping("/banks")
    public ResponseEntity<List<Bank>> getAllBanks(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
