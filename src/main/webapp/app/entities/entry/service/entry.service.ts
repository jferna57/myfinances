...
type RestOf<T extends IEntry | NewEntry> = Omit<T, 'entryDate'> & {
...
export class EntryService {
  ...
  getEntryIdentifier(entry: Pick<IEntry, 'id'>): string {
    return entry.id;
  }
  ...
  protected convertDateFromClient<T extends IEntry | NewEntry | PartialUpdateEntry>(entry: T): RestOf<T> {
    return {
      ...entry,
      entryDate: entry.entryDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEntry: RestEntry): IEntry {
    return {
      ...restEntry,
      entryDate: restEntry.entryDate ? dayjs(restEntry.entryDate) : undefined,
    };
  }
